...
![claim workflow](../../images/claim_workflow_cn.png)
...
[**附录A**](claim_spec_cn.md#a-可信声明模板)小节包含可信声明的一个示例，展示了企业颁发的收入证明的声明模板。
...
-  **clm** 指明了可信声明内容
-  **clm-rev** 指明了可信声明撤销方式。支持的方式在[附录C](claim_spec.md#C-Revocation).
...
撤销列表需要包含被签发者撤销的可信声明id，撤销时间。使用该机制，可信声明必须包含撤销列表的url，并且类型必须被指定为`RevocationList`。
```
    "clm-rev": {
        "type": "RevocationList",
...
- 存证合约撤销
 撤销信息通过存证合约的`Revoke`函数被记录在存证合约里。为了确认一个可信声明是否已撤销，标准方法是调用`GetStatus`函数。更多信息，请参阅[1.3节](claim_spec.md#1.3-attest-contract)。
```
    "clm-rev": {
        "type": "AttestContract",
        "addr": "8055b362904715fd84536e754868f4c8d27ca3f6"
